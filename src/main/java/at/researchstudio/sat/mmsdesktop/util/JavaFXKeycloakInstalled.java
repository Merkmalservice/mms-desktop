...
public class JavaFXKeycloakInstalled {
    ...
    public void login()
            throws IOException, ServerRequest.HttpFailure, VerificationException,
                    InterruptedException, OAuthErrorException, URISyntaxException {
    ...
    public void login(PrintStream printer, Reader reader)
            throws IOException, ServerRequest.HttpFailure, VerificationException,
                    InterruptedException, OAuthErrorException, URISyntaxException {
        if (isDesktopSupported()) {
            loginDesktop();
    ...
    public void loginDesktop()
            throws IOException, VerificationException, OAuthErrorException, URISyntaxException,
                    ServerRequest.HttpFailure, InterruptedException {
        JavaFXKeycloakInstalled.CallbackListener callback =
                new JavaFXKeycloakInstalled.CallbackListener();
        callback.start();
    ...
    protected JavaFXKeycloakInstalled.Pkce generatePkce() {
        return JavaFXKeycloakInstalled.Pkce.generatePkce();
    }
    ...
    private void logoutDesktop() throws IOException, URISyntaxException, InterruptedException {
        ...
        hostServices.showDocument(logoutUrl);
    ...
    public void loginManual(PrintStream printer, Reader reader)
            throws IOException, ServerRequest.HttpFailure, VerificationException {
    ...
    public static class Console {
        ...
        static JavaFXKeycloakInstalled.Console SINGLETON = new JavaFXKeycloakInstalled.Console();
        ...
        public JavaFXKeycloakInstalled.Console format(String fmt, Object... args) {
            if (console == null) {
        ...
        public JavaFXKeycloakInstalled.Console printf(String format, Object... args) {
            if (console == null) {
    ...
    public static JavaFXKeycloakInstalled.Console console() {
        return JavaFXKeycloakInstalled.Console.SINGLETON;
    }
    ...
    /**
     * Experimental proprietary WWW-Authentication challenge protocol. WWW-Authentication:
     * X-Text-Form-Challenge callback="{url}" param="{param-name}" label="{param-display-label}"
     *
     * @param redirectUri
     * @return
     * @throws IOException
    ...
    public static class Pkce {
        ...
        public static JavaFXKeycloakInstalled.Pkce generatePkce() {
            try {
                String codeVerifier =
                        new RandomString(PKCE_CODE_VERIFIER_MAX_LENGTH, new SecureRandom())
                                .nextString();
                String codeChallenge = generateS256CodeChallenge(codeVerifier);
                return new JavaFXKeycloakInstalled.Pkce(codeVerifier, codeChallenge);
            } catch (Exception ex) {
