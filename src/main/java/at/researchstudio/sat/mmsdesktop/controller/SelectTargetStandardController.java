...
public class SelectTargetStandardController implements Initializable {
    ...
    public SelectTargetStandardController(ReactiveStateService stateService) {
        ...
        this.stateService.getConvertState().getTargetStandardState().projectProperty().bind(selectedProject);
        this.stateService.getConvertState().getTargetStandardState().targetStandardProperty().bind(selectedStandard);
    }
    ...
    public void initialize(URL location, ResourceBundle resourceBundle) {
        this.projectList
                        ...
                        .addListener((observable, deSelected, selected) -> {
                            if (selected != null) {
                                standardList.getItems().clear();
                                standardList.getItems().addAll(selected.getStandards());
                                selectedProject.set(selected);
                                stateService.getConvertState().getTargetStandardState()
                                                .stepTargetStandardStatusProperty().set(
                                                ProcessState.STEP_ACTIVE);
                            }
                        });
        this.projectList.setCellFactory(param -> new JFXListCell<>() {
        ...
        this.projectList.setConverter(new StringConverter<>() {
        ...
        this.standardList.setCellFactory(param -> new JFXListCell<>() {
            @Override protected void updateItem(Standard item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
                    setGraphic(null);
                    setText(null);
        ...
        this.standardList.setConverter(new StringConverter<>() {
            @Override public String toString(Standard standard) {
                if (standard == null){
                    return null;
                }
            ...
            @Override public Standard fromString(String string) {
                return standardList.getItems()
                                .stream()
        ...
        this.standardList
                        ...
                        .addListener((observable, deSelected, selected) -> {
                            ...
                            stateService.getConvertState().getTargetStandardState().stepTargetStandardStatusProperty()
                                            .set(
                                                            ProcessState.STEP_COMPLETE);
                        });
        this.reloadButton.disableProperty().bind(not(stateService.getLoginState().loggedInProperty()));
    }
    ...
    public void handleLoadProjectsAction(ActionEvent actionEvent) {
        ...
        URL jsonUrl = Resources.getResource("graphql/query-projects.json");
        try {
            String queryString = Resources.toString(jsonUrl, StandardCharsets.UTF_8);
