...
public class IFC2ModelConverter {
    ...
    public static Model readFromFile(File ifcFile, TaskProgressListener taskProgressListener)
        ...
        Thread modelGeneratorThread =
                        new Thread(
                                        () -> {
                                            File tempDirectory = new File("c:/tmp/ifc2hdt/" + System.currentTimeMillis());
                                            tempDirectory.mkdirs();
                                            tempDirectory.deleteOnExit();
                                            //Model model = collectAsHdtModel(taskProgressListener, sinkToIterator,
                                            //                tempDirectory);
                                            //Model model = collectAsOrdinaryModel(taskProgressListener, sinkToIterator);
                                            Model model = collectAsHdtModel(taskProgressListener, sinkToIterator,
                                                            tempDirectory);
                                            modelRef.set(model);
    ...
    private static Model collectAsOrdinaryModel(TaskProgressListener taskProgressListener,
        ...
        while(it.hasNext()){
            if (++i % 1000000 == 0){
                taskProgressListener.notifyProgress("Collecting triples", String.format("Collected %d triples", i),0);
            }
            graph.add(it.next());
        }
    ...
    private static Model collectAsHdtModel(TaskProgressListener taskProgressListener, SinkToIterator sinkToIterator, File tempDirectory) {
        ...
        try {
            ...
            HDT hdt = HDTManager.generateHDT(new Iterator<TripleString>() {
                @Override public boolean hasNext() {
                    ...
                    if (!doesHaveNext){
                        ...
                        taskProgressListener.notifyProgress(compressTaskName, "Compression in progress", 0);
                    }
                    return doesHaveNext;
                }
            ...
            }, BASE_URI, new HDTSpecification(), new ProgressListener() {
                @Override public void notifyProgress(float level, String message) {
                    taskProgressListener.notifyProgress(collectTaskName, message, level);
                }
            });
    ...
    private static Model collectAsHdtModelMapReduce(TaskProgressListener taskProgressListener, SinkToIterator sinkToIterator,
                    File tempDirectory) {
        try {
            StreamToHdt streamToHdt = new StreamToHdt(taskProgressListener);
            Streams.stream(sinkToIterator.iterator())
                            //.parallel()
                            .forEach(tripleString -> streamToHdt.addTriple(tripleString));
            ...
            Optional<HDT> resultOpt = chunks
                .stream()
                //.parallel()
                .map(HdtChunkWriter::getHdt)
                .reduce((left, right) -> {
                    try {
                        if (left == null || left.getTriples().getNumberOfElements() == 0) {
                            return right;
                        }
                        if (right == null || right.getTriples().getNumberOfElements() == 0){
                            return left;
                        }
                        int reducerStep = reduceSteps.incrementAndGet();
                        ProgressListener listener = new ProgressListener() {
                            @Override public void notifyProgress(
                                            float level,
                                            String message) {
                                float reduceLevel =  (float) reducerStep / (float) totalReduceSteps;
                                if (taskProgressListener != null){
                                    taskProgressListener
                                                    .notifyProgress("Reduce step " + reducerStep, message, reduceLevel);
                                }
                            }
                        };
                        File location = new File(tempDirectory, UUID.randomUUID().toString());
                        location.mkdir();
                        HDTImpl merged = new HDTImpl(new HDTSpecification());
                        merged.cat(location.getAbsolutePath()+"/", left, right, listener);
                        left.close();
                        right.close();
                        return merged;
                    } catch (Exception e) {
                        throw new RuntimeException("Error combining chunks ", e);
                    }
                });
                        ...
                        if (this.next == null) {
                            ...
                            if (timeouts > 1) {
                                ...
                                logger.warn(
                                                ...
                                                backoffTimeout);
                                logger.warn(
                                                "The triple stream is not closed yet, which means one of the follwing:");
                                logger.warn(
    ...
    private static class HdtChunkWriter {
        ...
        public HdtChunkWriter(long maxSize, String baseUri, ProgressListener listener, int chunkIndex) throws IOException {
        ...
        public void addTriple(Triple triple) throws IOException {
            ...
            num++;
            size+=ts.getSubject().length()+ts.getPredicate().length()+ts.getObject().length()+4;  // Spaces and final dot
    ...
    private static TripleString toTripleString(Triple triple) {
        ...
        TripleString ts = new TripleString(subject, predicate, object);
        return ts;
    }
    ...
    private static class StreamToHdt {
        ...
        Set<HdtChunkWriter> finishedChunks;
        ...
        private HdtChunkWriter getChunkWriterAndPossiblySwapForNew() throws Exception {
            ...
            */
            ...
            int chunkIndex = chunks.incrementAndGet();
            writer = new HdtChunkWriter(MAX_CHUNK_SIZE, BASE_URI, new ProgressListener() {
                @Override public void notifyProgress(float level, String message) {
                    if (taskProgressListener != null){
                        taskProgressListener.notifyProgress("HDT chunk " + chunkIndex, message, level);
                    }
