package at.researchstudio.sat.merkmalservice.support.progress;
...
public class TaskProgressReporter {
    ...
    private AtomicReference<Double> position = new AtomicReference<>((double) 0);
    ...
    private MessageGenerator messageGenerator =
            progressData ->
                    String.format("Step %d of %d", progressData.getStep(), progressData.getSteps());

    public static ProgressReporterBuilder builder(
            TaskProgressListener listener, double targetValue) {
    ...
    private void setup() {
        ...
        if (this.progressListener == null) {
            logger.info(
                    "ProgressReporter for task '{}', targetValue {} got null passed for its ProgressListener - no progress will be reported",
                    taskName,
                    targetValue);
        }
    }
    ...
    public void advanceBy(double relativeValue) {
        ...
        this.position.updateAndGet(d -> d + relativeValue);
        ...
        if (isNewstep()) {
            synchronized (this) {
                if (isNewstep()) {
                    ...
                    this.step =
                            (long)
                                    (Math.floor(this.position.get() - this.startValue)
                                            / this.stepSize);
    ...
    private void reportProgress() {
        progressListener.notifyProgress(
                this.taskName,
                messageGenerator.generateProgressMessage(progressData),
    ...
    public class ProgressData {
        ...
        private double getProgressBetween0And1() {
            return (this.getPosition() - this.getStartValue())
                    / (this.getTargetValue() - this.getStartValue());
        }
    }
}
