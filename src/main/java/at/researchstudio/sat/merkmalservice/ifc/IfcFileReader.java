...
public class IfcFileReader {
    ...
    public static ParsedIfcFile processIfcLines(
        ...
        // collect propertySets
        List<IfcPropertySetLine> propertySetLines = new ArrayList<>();
        for (IfcLine line :
                        ifcLinesGrouped.getOrDefault(IfcPropertySetLine.class, Collections.emptyList())) {
            IfcPropertySetLine ifcPropertySetLine = (IfcPropertySetLine) line;
            propertySetLines.add(ifcPropertySetLine);
        }
        // collect SI units
        for (IfcLine line :
        ...
        for (IfcLine line :
            ...
            IfcProperty tempProp =
                            new IfcProperty(propertyLine.getId(), propertyLine.getName(), IfcPropertyType.VALUELIST);
            IfcProperty prop =
                            extractedProperties.stream()
                                            .filter(tempProp::equals)
                                            .findAny()
            ...
            for (Integer propertyId : propertySetLine.getPropertyIds()) {
                if (propertyPropertySetsMap.get(propertyId) != null
                                ...
                                propertyPropertySetsMap.get(propertyId).stream()
                                                .filter(ps -> propertySetLine.getName().equals(ps.getName())).findAny()
                                                .orElse(null) != null) {
                    // allready added
                } else {
                    propertyPropertySetsMap.computeIfAbsent(propertyId, k -> new HashSet<>())
                                    .add(new PropertySet(null, propertySetLine.getName(),
                                                    ...
                                                    .map(
                                                                    ifcProperty -> {
                                                                        ...
                                                                        f.setDescriptionFromUniqueValues(
                                                                                        ifcProperty.getExtractedUniqueValues());
                                                                        f.setPropertySets(propertyPropertySetsMap
                                                                                        .get(ifcProperty.getId()));
                                                                        return f;
                                                                    })
                                                    .collect(Collectors.toList()));
                    break;
